...
description: 日本語の技術文書・書籍原稿を、人間が読む成果物として執筆・推敲するための文章規範。保存・公開・継続参照を前提とする技術書の章、記事、解説、設計文書などを作成またはリライトするときに使用する。質問への通常の会話応答や、使い捨ての文章・短いメモには自動適用しない。ただし、ユーザーが本スキルの使用を明示した場合は、用途や長さにかかわらず必ず使用する。表記、段落と論証、読み手の負荷、視点と語り、冗長さを整える。
---
...
- 術語・訳語は、その分野で慣用されている語を選ぶ（プッシュ通知を送ることを「配送」と呼ばず、「配信」と呼ぶ。container image は「画像」と訳さず、「コンテナイメージ」と表記する）。意味の近い漢語を一般語の感覚で充てない。
