...
規律の基準は [referent-before-label](referent-before-label.md) rule が持つ。
対象作業ではこの rule を必ず読む。
...
- 保存先: 作業ディレクトリの `referent-table-<slug>.md` (成果物が `output/` 配下なら
  同じディレクトリに置く)。
- 同名ファイルが存在する場合は上書きせず、`referent-table-<slug>-2.md`、`-3.md` のように
  未使用の最小連番を付ける。
...
- **候補語は具体対象と役割の右側に固定。具体対象と役割が埋まるまで候補語欄は空欄のまま**にする。
  語を先に決めて対象を後付けする順序を、書式で不可能にするためである。
- 役割は原則として次から一つだけ選ぶ: `主体 / 構成要素 / 制約 / 開始条件 / 状態 / 事象 /
  値 / 記録 / 根拠 / 原因 / 仮説 / 判断 / 結果 / 目的 / 手段`。どれにも当てはまらない場合は
